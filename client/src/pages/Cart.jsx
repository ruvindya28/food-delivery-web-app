...
const Cart = () => {
     ...
     const {products,currency,cartItems,removeFromCart,getCartCount,updateCartItem,navigate,getCartTotal} = useAppContext()
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={() => removeFromCart()} className="cursor-pointer mx-auto">
                            <img src={assets.refresh_icon} alt='remove' className='inline-blockw-6 h-6' />
                        </button>
                    </div>)
                )}
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        <p className="text-gray-500">{setSelectedAddress ? `${selectedAddress.street}, ${selectedAddress.city}, ${selectedAddress.state}, ${selectedAddress.country}` : 'No address found'}</p>
                        <button onClick={() => setShowAddress(!showAddress)} className="text-indigo-500 hover:underline cursor-pointer">
                            Change
                        </button>
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                { address.map((adress, index)=>( <p onClick={() => {setSelectedAddress(adress); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
